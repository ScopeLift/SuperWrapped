...
contract ConstantSumHook is BaseHook, SafeCallback {
  ...
  function _beforeSwap(
    ...
    // tokens are always swapped 1:1, so use amountSpecified to determine both input and output
    // amounts
    uint256 amount =
    ...
    //   unspecifiedDelta = negative, to offset the credit of the output token paid by the hook
    // (positive delta)
    // exact output:
